...
nbatlas_cdend_celltypes <- c("cDC1", "cDC2/DC3", "Migratory cDC")
...
nbatlas_myeloid_celltypes <- c(
  nbatlas_nk_celltypes, 
  nbatlas_cdend_celltypes,
  nbatlas_mono_celltypes,
  "Macrophage",
  "Neutrophil"
)
...
Use the non-Zoom labels:
```{r}
annotation_df <- annotation_df |>
  dplyr::mutate(
    # make sure not to overwrite an existing label!
    final_label = dplyr::case_when(
      is.na(final_label) & singler %in% nbatlas_t_celltypes & scanvi %in% nbatlas_t_celltypes ~ "T cell",
      is.na(final_label) & singler %in% nbatlas_nk_celltypes & scanvi %in% nbatlas_nk_celltypes ~ "NK cell",
      is.na(final_label) & singler %in% nbatlas_myeloid_celltypes & scanvi %in% nbatlas_myeloid_celltypes ~ "Myeloid",
      .default = final_label
    )
  ) 


sum(!is.na(annotation_df$final_label))/nrow(annotation_df) # 0.8344759
```


> If the SingleR and scANVI labels are not in the same family and one of them matches with a consensus cell type then I would label it.



## Annotation by Stephanie
